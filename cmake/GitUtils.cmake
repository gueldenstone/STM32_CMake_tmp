...
function(git_clone)
    ...
    cmake_parse_arguments(
            ...
            "PROJECT_NAME;GIT_URL;GIT_TAG;GIT_BRANCH;GIT_COMMIT;DIRECTORY;TO_CLONE_DIRS"      # list of names of mono-valued arguments
    ...
    if(NOT PARGS_QUIET)
        message(STATUS "downloading/updating ${PARGS_PROJECT_NAME}/${PARGS_TO_CLONE_DIRS}")
    endif()

    # set SubDirs to be cloned
    if(NOT PARGS_TO_CLONE_DIRS)
        message(STATUS "no subdirectories given")
        set(CLONE_ALL TRUE)
    elseif(PARGS_TO_CLONE_DIRS)
        message(STATUS "only cloning ${PARGS_TO_CLONE_DIRS}")
        set(CLONE_SPARSE TRUE)
    endif()
    ...
    if(CLONE_SPARSE)
        if(EXISTS ${${SOURCE_DIR}})
            ...
            execute_process(
                    ...
                    COMMAND             echo "${PARGS_TO_CLONE_DIRS}" >> .git/info/sparse-checkout
                    COMMAND             ${GIT_EXECUTABLE} pull origin master
                    COMMAND             ${GIT_EXECUTABLE} submodule update --remote
                    WORKING_DIRECTORY   ${${SOURCE_DIR}}
                    OUTPUT_VARIABLE     git_output)
            ...
            execute_process(
                    ...
                    WORKING_DIRECTORY   ${${SOURCE_DIR}}
                    OUTPUT_VARIABLE     git_output)
            message(STATUS "inititalized .git")
            execute_process(
                    COMMAND             ${GIT_EXECUTABLE} remote add -f origin ${PARGS_GIT_URL}
                    WORKING_DIRECTORY   ${${SOURCE_DIR}}
                    OUTPUT_VARIABLE     git_output)
            message(STATUS "added remote")
            execute_process(
                    COMMAND             ${GIT_EXECUTABLE} config core.sparseCheckout true
                    WORKING_DIRECTORY   ${${SOURCE_DIR}}
                    OUTPUT_VARIABLE     git_output)
            message(STATUS "configure sparse cloning with ${PARGS_TO_CLONE_DIRS} in ${${SOURCE_DIR}}/.git/info/sparse-checkout")
            file(WRITE ${${SOURCE_DIR}}/.git/info/sparse-checkout "${PARGS_TO_CLONE_DIRS}")
